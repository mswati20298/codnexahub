...
export default function DateFormatConverter() {
  ...
  return (
    <div>
      ...
      />
      {!isValid ? (
        <p className="text-red-400 text-sm">Couldn't parse this as a date.</p>
      ) : (
        <div className="space-y-2">
          {FORMATS.map((fmt) => (
            <div key={fmt} className="flex justify-between items-center bg-slate-800 border border-slate-700 rounded-lg px-4 py-2">
              <span className="text-xs text-slate-400">{fmt}</span>
              <code className="text-sm text-indigo-400">{formatDate(date, fmt)}</code>
            </div>
          ))}
        </div>
      )}
