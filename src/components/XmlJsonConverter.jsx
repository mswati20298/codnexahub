  ...
  return (
    <div>
      <div className="flex gap-2 mb-4">
        <button onClick={() => { setMode("xml-to-json"); setInput(SAMPLE_XML); }} className={`text-sm px-3 py-1.5 rounded-lg border ${mode === "xml-to-json" ? "bg-emerald-600 border-emerald-500 text-white" : "bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300"}`}>XML → JSON</button>
        <button onClick={() => { setMode("json-to-xml"); setInput('{"user":{"name":"Alice","age":30}}'); }} className={`text-sm px-3 py-1.5 rounded-lg border ${mode === "json-to-xml" ? "bg-emerald-600 border-emerald-500 text-white" : "bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300"}`}>JSON → XML</button>
      </div>
