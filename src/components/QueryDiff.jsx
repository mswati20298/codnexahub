...
const SAMPLE_A = "SELECT id, name\nFROM users\nWHERE status = 'active';";
const SAMPLE_B = "SELECT id, name, email\nFROM users\nWHERE status = 'active'\nORDER BY name;";
  ...
  return (
    <div>
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        <textarea value={a} onChange={(e) => setA(e.target.value)} rows={7} className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-xs" />
        <textarea value={b} onChange={(e) => setB(e.target.value)} rows={7} className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-xs" />
      </div>
