  ...
  return (
    <div>
      <label className="flex flex-col gap-1 text-sm text-slate-300 mb-6 max-w-xs">
        ...
        <input type="number" min="0" value={gbPerMonth} onChange={(e) => setGbPerMonth(Number(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
      </label>
          ...
          <tbody>
            {results.map((p) => (
              <tr key={p.id} className="border-b border-slate-800/60">
                ...
                <td className="py-2 pr-4 text-slate-400">${p.pricePerGb.toFixed(3)}</td>
      ...
      <p className="text-xs text-slate-500 mt-4">"Egress fees" are one of the most underestimated cloud costs — moving data out is often far pricier than storing it.</p>
    </div>
  );
}
