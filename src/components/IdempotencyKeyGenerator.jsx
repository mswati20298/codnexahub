...
export default function IdempotencyKeyGenerator() {
  ...
  return (
    <div>
      ...
      <button onClick={regenerate} className="text-sm px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white">Generate new</button>
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">Send this as an Idempotency-Key header on POST/PUT requests (e.g. payments) so retries of the same request don't create duplicate side effects. Generate a new key per unique operation, not per retry.</p>
    </div>
  );
}
