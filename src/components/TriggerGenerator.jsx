    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Trigger name
          <input value={triggerName} onChange={(e) => setTriggerName(e.target.value)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Table
        ...
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Event
          <select value={event} onChange={(e) => setEvent(e.target.value)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100">
      ...
      <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300 mb-4">
        Trigger body
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={3} className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-xs" />
      </label>
      ...
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">Syntax shown is MySQL-style — PostgreSQL requires a separate function + trigger definition. Adjust for your specific database.</p>
    </div>
  );
}
