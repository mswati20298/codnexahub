...
export default function BoxShadowGenerator() {
  ...
  return (
    <div>
      <div className="w-full h-40 flex items-center justify-center bg-slate-100 dark:bg-slate-800 rounded-lg mb-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
          <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="w-10 h-10 rounded border border-slate-300 dark:border-slate-700 bg-transparent" />
          Shadow color
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Opacity: {opacity}%
          <input type="range" min="0" max="100" value={opacity} onChange={(e) => setOpacity(Number(e.target.value))} className="accent-emerald-500" />
        </label>
      </div>
