  ...
  return (
    <div>
      ...
      <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300 mb-4">
        ...
        <textarea value={setFields} onChange={(e) => setSetFields(e.target.value)} rows={4} className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-xs" />
      </label>
      <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300 mb-4">
        WHERE condition
        <input value={where} onChange={(e) => setWhere(e.target.value)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-sm" />
      </label>
      ...
      <p className="text-xs text-amber-600 dark:text-amber-400 mt-4">Always double-check your WHERE clause — an UPDATE without one affects every row in the table.</p>
    </div>
  );
}
