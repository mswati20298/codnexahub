    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-4 mb-6">
        ...
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          ...
          <input type="number" min="0" value={avgTokensPerRequest} onChange={(e) => setAvgTokensPerRequest(Number(e.target.value) || 0)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          API price ($/1M tokens, blended)
          <input type="number" min="0" value={apiPricePerM} onChange={(e) => setApiPricePerM(Number(e.target.value) || 0)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Self-host GPU cost ($/month)
          <input type="number" min="0" value={gpuMonthlyRent} onChange={(e) => setGpuMonthlyRent(Number(e.target.value) || 0)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
      </div>
      <div className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg p-4 mb-3">
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Current monthly API cost</p>
        <p className="text-2xl font-semibold text-emerald-600 dark:text-emerald-400">${results.monthlyApiCost.toFixed(2)}</p>
      </div>
      <p className={`text-sm ${results.isBreakEven ? "text-emerald-600 dark:text-emerald-400" : "text-slate-700 dark:text-slate-300"}`}>
        {results.isBreakEven
      ...
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">Simplified comparison — excludes engineering time, reliability, and scaling complexity of self-hosting.</p>
    </div>
  );
}
