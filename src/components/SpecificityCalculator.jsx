        ...
        {[{ label: "Selector A", value: selectorA, set: setSelectorA, spec: specA }, { label: "Selector B", value: selectorB, set: setSelectorB, spec: specB }].map((s) => (
          <div key={s.label}>
            <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
              {s.label}
              <input value={s.value} onChange={(e) => s.set(e.target.value)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-sm" />
            </label>
            <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1">Specificity: ({s.spec.ids}, {s.spec.classes}, {s.spec.elements})</p>
          </div>
        ))}
      </div>
      <p className="text-sm text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-3">
