...
export default function CustomPropertiesGenerator() {
  ...
  return (
    <div>
      <div className="space-y-2 mb-4">
        {vars.map((v, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <input value={v.name} onChange={(e) => update(i, "name", e.target.value)} placeholder="name" className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-slate-100 text-sm" />
            <span className="text-slate-500">:</span>
            <input value={v.value} onChange={(e) => update(i, "value", e.target.value)} placeholder="value" className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 text-slate-100 text-sm" />
            <button onClick={() => remove(i)} className="text-xs text-red-400 hover:text-red-300">Remove</button>
          </div>
        ))}
      </div>
      <button onClick={add} className="text-sm px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 hover:border-indigo-500 text-slate-300 mb-4">+ Add variable</button>
      ...
      <p className="text-xs text-slate-500 mt-4">Use with var(--{"{name}"}) anywhere in your CSS — custom properties can also be scoped to specific selectors, not just :root.</p>
    </div>
  );
}
