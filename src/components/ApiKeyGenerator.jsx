    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Prefix (optional)
          <input value={prefix} onChange={(e) => setPrefix(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 font-mono" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Length
          <input type="number" min="8" max="64" value={length} onChange={(e) => setLength(Number(e.target.value) || 8)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
      </div>
      ...
      <p className="text-xs text-slate-500 mt-4">Generated using your browser's cryptographically secure random number generator.</p>
    </div>
  );
}
