    ...
    <div className="space-y-1">
      {FUNCTIONS.map((f) => (
        <div key={f.fn} className="flex gap-3 bg-slate-800 border border-slate-700 rounded-lg px-4 py-2">
          <code className="text-indigo-400 w-40 shrink-0">{f.fn}</code>
          <span className="text-sm text-slate-300">{f.desc}</span>
        </div>
      ))}
      <p className="text-xs text-slate-500 mt-4">Aggregate functions require GROUP BY when combined with non-aggregated columns in SELECT — any selected column not in an aggregate must appear in GROUP BY.</p>
    </div>
  );
}
