    ...
    <div>
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.keys(PRESETS).map((key) => (
          <button key={key} onClick={() => setPreset(key)} className={`text-sm px-3 py-1.5 rounded-lg border ${preset === key ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}>{key}</button>
        ))}
      </div>
      <div className="grid sm:grid-cols-3 gap-4 mb-4">
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Duration (s)
          <input type="number" min="0.1" step="0.1" value={duration} onChange={(e) => setDuration(Number(e.target.value) || 0.1)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Timing function
        ...
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Iteration count
          <input value={iteration} onChange={(e) => setIteration(e.target.value)} placeholder="1 or infinite" className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
      </div>
