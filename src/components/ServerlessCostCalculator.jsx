    ...
    <div>
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Requests/month
          <input type="number" min="0" value={requestsPerMonth} onChange={(e) => setRequestsPerMonth(Number(e.target.value) || 0)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Avg duration (ms)
          <input type="number" min="1" value={avgDurationMs} onChange={(e) => setAvgDurationMs(Number(e.target.value) || 1)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
          Memory (MB)
          <input type="number" min="128" step="64" value={memoryMb} onChange={(e) => setMemoryMb(Number(e.target.value) || 128)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
        </label>
      </div>

      <div className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg p-4 mb-3">
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Estimated monthly cost (after free tier)</p>
        <p className="text-2xl font-semibold text-emerald-600 dark:text-emerald-400">${results.totalCost.toFixed(2)}</p>
      </div>
