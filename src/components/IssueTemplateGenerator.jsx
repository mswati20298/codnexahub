  ...
  return (
    <div>
      <div className="flex gap-2 mb-4">
        {["bug", "feature"].map((t) => (
          <button key={t} onClick={() => setType(t)} className={`text-sm px-3 py-1.5 rounded-lg border capitalize ${type === t ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}>{t} report</button>
        ))}
      </div>
