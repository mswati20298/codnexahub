      ...
      {[
        ...
        <label key={f.label} className="flex flex-col gap-1 text-sm text-slate-300">
          ...
          <input
            ...
            className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
        </label>
      ))}
      {!values && <p className="text-red-400 text-sm">Invalid number.</p>}
    </div>
  );
}
