  ...
  return (
    <div>
      ...
      <div onClick={copy} className="cursor-pointer bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-4 hover:border-emerald-500">
        <pre className="text-sm text-emerald-600 dark:text-emerald-400 whitespace-pre-wrap"><code>{sql}</code></pre>
      </div>
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">A composite primary key is common for many-to-many join tables — the combination of columns must be unique, even if no single column is.</p>
    </div>
  );
}
