  ...
  return (
    <div>
      <label className="flex flex-col gap-1 text-sm text-slate-300 mb-4 max-w-xs">
        View name
        <input value={viewName} onChange={(e) => setViewName(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
      </label>
      ...
      <label className="flex flex-col gap-1 text-sm text-slate-300 mb-4">
        ...
        <textarea value={query} onChange={(e) => setQuery(e.target.value)} rows={5} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 font-mono text-xs" />
      </label>
