...
export default function ConnectionStringParser() {
  ...
  return (
    <div>
      ...
      />
      {!parsed ? (
        <p className="text-red-600 dark:text-red-400 text-sm">Couldn't parse this — check the format (e.g. protocol://user:pass@host:port/db).</p>
      ) : (
        <div className="grid sm:grid-cols-2 gap-3">
          {Object.entries(parsed).map(([key, value]) => (
            <div key={key} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg p-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-1 capitalize">{key}</p>
              <p className="font-mono text-sm text-slate-900 dark:text-slate-100 break-all">{value}</p>
            </div>
          ))}
        </div>
      )}
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">Password is masked for display. Parsing happens entirely in your browser.</p>
    </div>
  );
}
