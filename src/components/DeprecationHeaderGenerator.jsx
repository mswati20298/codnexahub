    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Sunset date
          <input type="date" value={sunsetDate} onChange={(e) => setSunsetDate(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Migration guide URL
          <input value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
      </div>
      ...
      <p className="text-xs text-slate-500 mt-4">Based on the IETF Deprecation and Sunset HTTP header drafts — gives API consumers a machine-readable way to detect deprecated endpoints before they break.</p>
    </div>
  );
}
