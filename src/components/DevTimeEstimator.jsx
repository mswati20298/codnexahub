  ...
  return (
    <div>
      <div className="grid sm:grid-cols-2 gap-2 mb-4">
        {FEATURES.map((f) => (
          <label key={f.key} className="flex items-center gap-2 text-sm text-slate-300">
            ...
            {f.label} <span className="text-slate-500 text-xs">({f.hours}h)</span>
          </label>
        ))}
      </div>
      <div className="grid sm:grid-cols-2 gap-4 mb-6">
        ...
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Focused hours/day
          <input type="number" min="1" value={hoursPerDay} onChange={(e) => setHoursPerDay(Number(e.target.value) || 1)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
      </div>
      <div className="grid sm:grid-cols-3 gap-3">
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
          <p className="text-xs text-slate-400 mb-1">Base estimate</p>
          <p className="text-lg font-semibold text-slate-100">{results.baseHours}h</p>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
          <p className="text-xs text-slate-400 mb-1">With buffer</p>
          <p className="text-lg font-semibold text-indigo-400">{results.totalHours.toFixed(0)}h</p>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
          <p className="text-xs text-slate-400 mb-1">Working days</p>
          <p className="text-lg font-semibold text-slate-100">{results.days.toFixed(1)}</p>
        </div>
      </div>
      <p className="text-xs text-slate-500 mt-4">Rough starting estimates per feature — always adjust for your specific requirements and team experience.</p>
    </div>
  );
}
