...
export default function ConventionalCommitBuilder() {
  ...
  return (
    <div>
      ...
      <label className="flex flex-col gap-1 text-sm text-slate-300 mb-4">
        Body (optional)
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={3} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
      </label>
      <div onClick={copy} className="cursor-pointer bg-slate-950 border border-slate-800 rounded-lg p-4 hover:border-indigo-500">
        <pre className="text-sm text-indigo-400 whitespace-pre-wrap">{message}</pre>
      </div>
    </div>
  );
}
