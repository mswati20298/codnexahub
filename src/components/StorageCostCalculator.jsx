  ...
  return (
    <div>
      <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300 mb-6 max-w-xs">
        Storage (GB)
        <input type="number" min="0" value={gb} onChange={(e) => setGb(Number(e.target.value) || 0)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100" />
      </label>
          ...
          <tbody>
            {results.map((p) => (
              <tr key={p.id} className="border-b border-slate-200 dark:border-slate-800/60">
                ...
                <td className="py-2 pr-4 text-slate-500 dark:text-slate-400">${p.pricePerGb.toFixed(4)}</td>
      ...
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">Storage cost only — doesn't include egress/bandwidth charges, which can be significant. Check the Bandwidth Cost Calculator separately.</p>
    </div>
  );
}
