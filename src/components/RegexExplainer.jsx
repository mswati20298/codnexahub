  ...
  return (
    <div>
      <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300 mb-4">
        Regex pattern
        <input value={pattern} onChange={(e) => setPattern(e.target.value)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-sm" />
      </label>
      <label className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300 mb-4">
        Test string
        <input value={testString} onChange={(e) => setTestString(e.target.value)} className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 font-mono text-sm" />
      </label>
      {matchResult !== null && (
        <p className={`text-sm mb-4 ${matchResult ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>{matchResult ? "✓ Matches" : "✗ Does not match"}</p>
      )}
      <div className="space-y-1">
        {explanations.map((e) => (
          <div key={e.label} className="flex gap-3 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm">
            <code className="text-emerald-600 dark:text-emerald-400 w-16 shrink-0">{e.label}</code>
            <span className="text-slate-700 dark:text-slate-300">{e.desc}</span>
          </div>
        ))}
        {explanations.length === 0 && <p className="text-slate-500 dark:text-slate-500 text-sm">No recognized tokens found in this pattern.</p>}
      </div>
    </div>
  );
}
