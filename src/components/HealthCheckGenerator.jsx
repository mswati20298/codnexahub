    ...
    <div>
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.keys(SNIPPETS).map((l) => (
          <button key={l} onClick={() => setLang(l)} className={`text-sm px-3 py-1.5 rounded-lg border ${lang === l ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}>{l}</button>
        ))}
      </div>
      ...
      <p className="text-xs text-slate-500 mt-4">A good health check should verify critical dependencies (database, cache) are reachable, not just that the process is running.</p>
    </div>
  );
}
