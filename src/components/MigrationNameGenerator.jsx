    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        {[
          ...
          <button key={s.key} onClick={() => setStyle(s.key)} className={`text-sm px-3 py-1.5 rounded-lg border ${style === s.key ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}>{s.label}</button>
        ))}
      </div>
      <div onClick={copy} className="cursor-pointer bg-slate-950 border border-slate-800 rounded-lg p-4 hover:border-indigo-500">
        <code className="text-sm text-indigo-400 break-all">{filename}</code>
      </div>
    </div>
  );
}
