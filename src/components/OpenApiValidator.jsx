  ...
  return (
    <div>
      ...
      {error ? (
        ...
        <p className="text-emerald-600 dark:text-emerald-400 text-sm">No structural issues found.</p>
      ) : (
        <ul className="space-y-1">
          {issues.map((issue, i) => (
            <li key={i} className="text-sm text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2">{issue}</li>
          ))}
        </ul>
      )}
      <p className="text-xs text-slate-500 dark:text-slate-500 mt-4">Checks basic structural requirements only — not a full OpenAPI schema validator.</p>
    </div>
  );
}
