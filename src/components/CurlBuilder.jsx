  ...
  return (
    <div>
      ...
      <label className="flex flex-col gap-1 text-sm text-slate-300 mb-4">
        ...
        <textarea value={headers} onChange={(e) => setHeaders(e.target.value)} rows={3} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 font-mono text-xs" />
      </label>
      {method !== "GET" && (
        <label className="flex flex-col gap-1 text-sm text-slate-300 mb-4">
          Body (JSON)
