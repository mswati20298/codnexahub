    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          ...
          <input value={code} onChange={(e) => setCode(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 font-mono text-sm" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          HTTP status
          <input type="number" value={status} onChange={(e) => setStatus(Number(e.target.value) || 400)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
        ...
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Field with issue
          <input value={field} onChange={(e) => setField(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
        <label className="flex flex-col gap-1 text-sm text-slate-300">
          Issue detail
          <input value={detail} onChange={(e) => setDetail(e.target.value)} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100" />
        </label>
      </div>
      ...
      <p className="text-xs text-slate-500 mt-4">A consistent error shape across your whole API makes client-side error handling far simpler — this is one reasonable convention among several valid ones.</p>
    </div>
  );
}
