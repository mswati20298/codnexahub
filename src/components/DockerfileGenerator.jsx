    ...
    <div>
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.keys(TEMPLATES).map((key) => (
          <button key={key} onClick={() => setStack(key)} className={`text-sm px-3 py-1.5 rounded-lg border ${stack === key ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}>
            {key}
          </button>
        ))}
      </div>
