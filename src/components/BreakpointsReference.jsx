    ...
    <div>
      <div className="flex gap-2 mb-4">
        {Object.keys(FRAMEWORKS).map((f) => (
          <button key={f} onClick={() => setFramework(f)} className={`text-sm px-3 py-1.5 rounded-lg border ${framework === f ? "bg-indigo-600 border-indigo-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"}`}>{f}</button>
        ))}
      </div>
      <div className="space-y-1">
        {FRAMEWORKS[framework].map((bp) => (
          <div key={bp.name} className="flex justify-between items-center bg-slate-800 border border-slate-700 rounded-lg px-4 py-2">
            <span className="text-sm text-slate-100">{bp.name}</span>
            <code className="text-sm text-indigo-400">{bp.width}</code>
          </div>
        ))}
      </div>
    </div>
