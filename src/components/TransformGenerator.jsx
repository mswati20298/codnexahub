...
export default function TransformGenerator() {
  ...
  return (
    <div>
      <div className="w-full h-40 flex items-center justify-center bg-slate-800 rounded-lg mb-6 overflow-hidden">
        <div
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        {[
          ...
          <label key={f.label} className="flex flex-col gap-1 text-sm text-slate-300">
            ...
            <input type="range" min={f.min} max={f.max} step={f.step || 1} value={f.value} onChange={(e) => f.set(Number(e.target.value))} className="accent-indigo-500" />
          </label>
        ))}
      </div>
